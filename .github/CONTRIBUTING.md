...
Every time a commit is merged into master, a new patch release is [automatically drafted](https://github.com/linode/linode-filestorage-csi-driver/actions/workflows/release-drafter.yml) with a changelog. You can modify (tag name, changelog, title, etc.) and publish the release via the [releases page](https://github.com/linode/linode-filestorage-csi-driver/releases).

When a release is published, the [release workflow](https://github.com/linode/linode-filestorage-csi-driver/actions/workflows/release.yml) builds and pushes the docker image to Docker Hub.
